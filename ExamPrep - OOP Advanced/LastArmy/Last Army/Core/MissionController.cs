...
public class MissionController
{
    ...
    public string PerformMission(IMission mission)
    {
        ...
        for (int i = 0; i < currentMissionsCount; i++)
        {
            ...
            if (successful)
            {
                sb.AppendLine(string.Format(OutputMessages.MissionSuccessful + currentMission.Name).Trim());
            }
            else
            {
                this.missionQueue.Enqueue(currentMission);
                sb.AppendLine(string.Format(OutputMessages.MissionOnHold + currentMission.Name).Trim());
            }
        }
